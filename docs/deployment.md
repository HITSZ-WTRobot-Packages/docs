...
`Validation` 并确认通过。若同步仅报告 `retained`，则不得产生机器人提交或重新构建站点；observed
SHA 只保留在 Actions 运行记录中。发生发布内容变化时，部署重新构建必须使用同一机器人提交且不访问上游仓库，并验证受影响的软件包修订版本和固定源码链接已更新，未受影响的稳定路由仍然有效。
