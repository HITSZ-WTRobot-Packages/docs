...
`bun run generate:api` 只验证每个模块已提交的 `api-catalog.json`、清单校验和、模块身份、源码分支和
...
排除搜索索引。“驱动包”组从已提交 catalog 动态生成三级导航。每个模块是一个默认折叠的分组，包含模块 README 入口和按名称排序的软件包入口；访问模块或软件包页面时，Starlight 会自动展开对应模块并高亮当前页面。
